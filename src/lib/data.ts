...
function logErr(label: string, err: unknown) {
  if (err && typeof err === 'object') {
    ...
    console.error(label, JSON.stringify({ message: e.message, code: e.code, hint: e.hint, details: e.details }))
  } else {
...
export async function getTodayDrop(): Promise<DailyDrop | null> {
  ...
  try {
    ...
    if (rpcError) logErr('[drop] rpc error', rpcError)
    ...
    if (drop && !rpcError) {
      ...
      const { data, error: joinError } = await supabase
        ...
        .eq('id', dropId)
        .single()
      if (joinError) logErr('[drop] join error', joinError)
      return data
    }
    ...
    const { data, error: directError } = await supabase
      ...
      .eq('date', today)
      .single()
    if (directError) logErr('[drop] direct error', directError)
    return data ?? null
